...
class BannerTile(Gtk.FlowBoxChild):
    def __init__(self, pkginfo, installer, name, background, color, is_flatpak, show_verified, app_json, on_clicked_action):
...
class Application(Gtk.Application):
    ...
    def load_banner(self):
        ...
        arrow_box = Gtk.Box(orientation=Gtk.Orientation.HORIZONTAL, spacing=0)
        left_arrow = Gtk.Button.new_from_icon_name("go-previous-symbolic", Gtk.IconSize.SMALL_TOOLBAR)
        right_arrow = Gtk.Button.new_from_icon_name("go-next-symbolic", Gtk.IconSize.SMALL_TOOLBAR)
        left_arrow.set_size_request(20, 20)  # Adjust the size of the left arrow button
        right_arrow.set_size_request(20, 20)  # Adjust the size of the right arrow button

        dot_box = Gtk.Box(orientation=Gtk.Orientation.HORIZONTAL, spacing=5)
        dot_box.set_halign(Gtk.Align.CENTER)

        left_arrow.connect("clicked", self.on_arrow_clicked, stack, dot_box, -1)
        right_arrow.connect("clicked", self.on_arrow_clicked, stack, dot_box, 1)
        ...
        for app_json in json_array:
            ...
            is_flatpak = name.startswith("flatpak:")
            if is_flatpak:
                name = name.replace("flatpak:", "")
                pkg_type = installer.PKG_TYPE_FLATPAK
            else:
                pkg_type = installer.PKG_TYPE_APT

            pkginfo = self.installer.find_pkginfo(name, pkg_type)
            ...
            tile = BannerTile(pkginfo, self.installer, name, background, color, is_flatpak, show_verified, app_json, self.on_banner_clicked)
            ...
            dot_button.set_size_request(10, 10)  # Adjust the size of the dot buttons
            dot_button.set_image(Gtk.Image.new_from_icon_name("media-record-symbolic", Gtk.IconSize.MENU))
            dot_button.connect("clicked", self.on_dot_clicked, stack, len(stack.get_children()) - 1)
            dot_box.pack_start(dot_button, False, False, 0)
        
        arrow_box.pack_start(left_arrow, False, False, 0)
        arrow_box.pack_start(stack, True, True, 0)
        arrow_box.pack_start(right_arrow, False, False, 0)

        box.pack_start(arrow_box, True, True, 0)
    ...
    def on_arrow_clicked(self, button, stack, dot_box, increment):
        self.reset_slideshow_timer(stack, dot_box)
        visible_child = stack.get_visible_child()
        index = stack.get_children().index(visible_child)
        new_index = (index + increment) % len(stack.get_children())
        stack.set_visible_child_name(str(new_index))
        dot_box = stack.get_parent().get_parent().get_children()[-1]  # Get the dot_box from the parent of the arrow_box
    ...
    def update_dot_buttons(self, dot_box, current_index):
        for i, button in enumerate(dot_box.get_children()):
            if i == current_index:
                button.set_image(Gtk.Image.new_from_icon_name("media-record-symbolic", Gtk.IconSize.DND))
            else:
                button.set_image(Gtk.Image.new_from_icon_name("media-record-symbolic", Gtk.IconSize.MENU))
